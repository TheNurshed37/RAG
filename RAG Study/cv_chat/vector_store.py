...
def extract_structured_sections_with_docling(docling_result):
    """
    Use Docling's native structure to identify CV sections - SIMPLIFIED
    """
    structured_sections = {}
...
def remove_personal_info(text):
    """Remove personal information patterns from text - LESS AGGRESSIVE"""
    if not text:
        return text
...
def convert_pdf_with_docling_enhanced(pdf_path, candidate_data):
    ...
    try:
        ...
        logger.info(f"Successfully created SINGLE golden chunk for {candidate_data['candidate_name']}")
...
def convert_pdf_with_docling_enhanced_optimized(pdf_path, candidate_data, pre_processed_content):
    ...
    try:
        ...
        logger.info(f"Successfully created SINGLE golden chunk for {candidate_data['candidate_name']} using optimized processing")
        return golden_chunks
...
def add_to_faiss_index(pdf_path, file_hash, faiss_dir, hash_index_file):
    ...
    if file_hash in hash_index["file_hashes"]:
        ...
        return f"Skipped: '{candidate_data['candidate_name']}' already exists in vector store."
    ...
    return f"Single golden chunk created successfully for '{candidate_data['candidate_name']}' (ID: {candidate_data['candidate_id']})"